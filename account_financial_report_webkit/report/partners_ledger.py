...
class PartnersLedgerWebkit(report_sxw.rml_parse,
        ...
        if data['form']['result_selection'] == 'customer':
            ...
            if initial_balance_mode == 'initial_balance':
                initial_balance_lines = self._compute_partners_initial_balances(
                    accounts, event_ids, start_period, partner_filter=partner_ids,
                    exclude_reconcile=False)
    
            ledger_lines = self._compute_partner_ledger_lines(
                accounts, event_ids, main_filter, target_move, start, stop,
            ...
            for event_id in event_ids:
                ledger_lines_dict[event_id] = {}
                init_balance[event_id] = {}
                partners_order[event_id] = {}
                for account in objects:
                    ledger_lines_dict[event_id][account.id] = ledger_lines.get(event_id, {}).get(account.id, {})
                    init_balance[event_id][account.id] = initial_balance_lines.get(event_id, {}).get(account.id, {})
                    ...
                    ledg_lines_pids = ledger_lines[event_id].get(account.id, {}).keys()
                    if initial_balance_mode:
                        ...
                        init_balance[event_id][account.id] = {}
                        init_bal_lines_pids = []
    ...
    def _compute_partner_ledger_lines(self, accounts_ids, event_ids, main_filter,
        ...
        if event_ids:
            ...
            for event_id in event_ids:
                ...
                for acc_id in accounts_ids:
                    move_line_ids = self.get_partners_move_lines_ids(
                        acc_id, event_id, main_filter, start, stop, target_move,
            ...
            for acc_id in accounts_ids:
                move_line_ids = self.get_partners_move_lines_ids(
                    acc_id, False, main_filter, start, stop, target_move,
                    exclude_reconcile=False, partner_filter=partner_filter)
                if not move_line_ids:
                    continue
                for partner_id in move_line_ids:
...
HeaderFooterTextWebKitParser(
    'report.account.account_report_partners_ledger_without_event_webkit',
    'account.account',
    'addons/account_financial_report_webkit/report/templates/\
                                        account_report_partners_ledger_without_event.mako',
    parser=PartnersLedgerWebkit)
