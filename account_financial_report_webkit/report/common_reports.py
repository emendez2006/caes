...
class CommonReportHeaderWebkit(common_report_header):
    ...
    def _get_event_br(self, data):
        event_obj = self.pool.get('event.event')
        if data.get('form', {}).get('event_ids'):
            return self._get_info(data, 'event_ids', 'event.event')
        else:
            event_ids = event_obj.search(self.cr, self.uid, [])
            return self.pool.get('event.event').browse(self.cursor, self.uid, event_ids)
