...
class CommonPartnerBalanceReportHeaderWebkit(CommonBalanceReportHeaderWebkit,
    ...
    def _get_account_partners_details(self, account_by_ids, event_ids, main_filter,
        ...
        if event_ids:
            for event_id in event_ids:
                final_res[event_id] = {}
                partners_init_balances_by_ids[event_id] = {}
                res = {}
                for account_id, account_details in account_by_ids.iteritems():
                    partners_init_balances_by_ids[event_id].update(
                        self._get_partners_initial_balances(
                            account_id, event_id, start, initial_balance_mode,
                ...
                final_res[event_id] = res
        else:
            res = {}
            for account_id, account_details in account_by_ids.iteritems():
                partners_init_balances_by_ids.update(
                    self._get_partners_initial_balances(
                        account_id, False, start, initial_balance_mode,
                        partner_filter_ids=partner_filter_ids,
                        # we'll never exclude reconciled entries in the legal
                        # reports
                        exclude_reconcile=False))
    ...
    def _get_partners_initial_balances(self, account_ids, event_ids, start_period,
                                       initial_balance_mode,
        ...
        if initial_balance_mode == 'opening_balance':
            ...
            res = self._compute_partners_initial_balances(
                account_ids, event_ids, start_period, partner_filter_ids,
    ...
    def _get_partners_totals_account(self, filter_from, account_id, event_id, start,
                                     stop, target_move,
                                     partner_filter_ids=None,
        ...
        if event_id:
            sql_where += " AND account_move_line.event_id = %(event_id)s"
            search_params.update({'event_id': event_id})
        ...
        search_params.update({'account_id': account_id, 'event_id': event_id})
    ...
    def _get_partners_comparison_details(self, data, account_ids, event_id, target_move,
        ...
        if comparison_filter != 'filter_no':
            ...
            accounts_by_ids = self._get_account_details(
                account_ids, [event_id], target_move, fiscalyear, details_filter, start,
                stop, initial_balance_mode)

            partner_details_by_ids = self._get_account_partners_details(
                accounts_by_ids, [event_id], details_filter,
    ...
    def compute_partner_balance_data(self, data, filter_report_type=None):
        ...
        event_ids = self._get_form_param('event_ids', data)
        ...
        if data['form']['result_selection'] == 'customer':
            if not event_ids:
                event_ids = event_obj.search(self.cr, self.uid, [])
            ...
            for event_id in event_ids:
                event_dict[event_id] = {} 
                accounts_by_ids = self._get_account_details(
                    account_ids, [event_id], target_move, fiscalyear, main_filter, start, stop,
                    initial_balance_mode)
                event_dict[event_id] = accounts_by_ids
            
                partner_details_by_ids = self._get_account_partners_details(
                    accounts_by_ids, [event_id], main_filter, target_move, start, stop,
                ...
                init_balance_accounts[event_id] = {}
                comparisons_accounts[event_id] = {}
                partners_order_accounts[event_id] = {}
                partners_amounts_accounts[event_id] = {}
                debit_accounts[event_id] = {}
                credit_accounts[event_id] = {}
                balance_accounts[event_id] = {}
        
                for account in objects:
                    if not account.parent_id:  # hide top level account
                        continue
                    debit_accounts[event_id][account.id] = accounts_by_ids[account.id]['debit']
                    credit_accounts[event_id][account.id] = accounts_by_ids[account.id]['credit']
                    balance_accounts[event_id][account.id] = \
                        accounts_by_ids[account.id]['balance']
                    init_balance_accounts[event_id][account.id] = accounts_by_ids[
                        account.id].get('init_balance', 0.0)
                    partners_amounts_accounts[event_id][account.id] =\
                    ...
                    for comp_account_by_id in comp_accounts_by_ids:
                        ...
                        for partner_id, partner_values in \
                            ...
                            base_partner_balance = partners_amounts_accounts[event_id][account.id][partner_id]['balance']\
                                if partners_amounts_accounts[event_id].get(account.id)\
                                and partners_amounts_accounts[event_id].get(account.id)\
                    ...
                    comparisons_accounts[event_id][account.id] = comp_accounts
                    ...
                    partners_order_accounts[event_id][account.id] = \
