...
<html xmlns="http://www.w3.org/1999/xhtml">
    ...
    <body>
		...
		%for event in get_event(data):
	        %for current_account in objects:
	            <%
	            partners_order = partners_order_accounts[event.id][current_account.id]
	
	            # do not display accounts without partners
	            if not partners_order:
	                continue
	
	            comparisons = comparisons_accounts[event.id][current_account.id]
	            ...
	            current_partner_amounts = partners_amounts_accounts[event.id][current_account.id]
				...
				<div class="account_title bg" style="margin-top: 20px; font-size: 12px; width: 690px;">${event.name}</div>
