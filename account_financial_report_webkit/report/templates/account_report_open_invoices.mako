        ...
        %for event_id in get_event(data):
	        %for acc in objects:
	            %if 'grouped_ledger_lines' in acc:
	               <% fl = formatLang %>
	              <%include file="grouped_by_curr_open_invoices_inclusion.mako.html" args="account=acc,event=event_id,formatLang=fl"/>
	            %else:
	               <% fl = formatLang %>
	              <%include file="open_invoices_inclusion.mako.html" args="event=event_id,account=acc,formatLang=fl"/>
	            %endif
        	%endfor
        %endfor
    </body>
