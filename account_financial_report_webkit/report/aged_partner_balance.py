    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if data['form']['result_selection'] == 'customer':
            ...
            for event_id in event_ids:
                agged_lines_accounts[event_id] = {}
                agged_totals_accounts[event_id] = {}
                agged_percents_accounts[event_id] = {}
                for acc in self.objects:
                    agged_lines_accounts[event_id][acc.id] = {}
                    agged_totals_accounts[event_id][acc.id] = {}
                    agged_percents_accounts[event_id][acc.id] = {}
                    
                    for part_id, partner_lines in\
                            self.localcontext['ledger_lines'][event_id][acc.id].items():
        
                        aged_lines = self.compute_aged_lines(part_id,
                                                             partner_lines,
                                                             data)
                        if aged_lines:
                            agged_lines_accounts[event_id][acc.id][part_id] = aged_lines
                    agged_totals_accounts[event_id][acc.id] = totals = self.compute_totals(
                        agged_lines_accounts[event_id][acc.id].values())
                    agged_percents_accounts[event_id][acc.id] = self.compute_percents(totals)
...
HeaderFooterTextWebKitParser(
    'report.account.account_aged_trial_balance_without_event_webkit',
    'account.account',
    'addons/account_financial_report_webkit/report/templates/\
                                                    aged_trial_webkit_without_event.mako',
    parser=AccountAgedTrialBalanceWebkit,
)
