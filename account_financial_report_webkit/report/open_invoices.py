...
class PartnersOpenInvoicesWebkit(report_sxw.rml_parse,
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        if data['form']['result_selection'] == 'customer':
            ...
            ledger_lines_memoizer = self._compute_open_transactions_lines(
                account_ids, event_ids, main_filter, target_move, start, stop, date_until,
            ...
            for event_id in event_ids:
                ...
                init_balance[event_id] = {}
                partners_order[event_id] = {}
                init_balance_memoizer[event_id] = {}
                for account in objects:
                    ledger_lines[event_id][account.id] = ledger_lines_memoizer[event_id].get(account.id,
                                                                         {})
                    init_balance[event_id][account.id] = init_balance_memoizer[event_id].get(account.id,
                                                                         {})
                    ...
                    ledg_lines_pids = ledger_lines_memoizer[event_id].get(account.id, {}).keys()
                    non_null_init_balances = dict([
                        (ib, amounts) for ib, amounts
                        in init_balance[event_id][account.id].iteritems()
                        if amounts['init_balance']
                        or amounts['init_balance_currency']])
                    init_bal_lines_pids = non_null_init_balances.keys()
                    
                    partners_order[event_id][account.id] = self._order_partners(
    ...
    def _compute_open_transactions_lines(self, accounts_ids, event_ids, main_filter,
        ...
        if event_ids:
            ...
            for event_id in event_ids:
                ...
                for account_id in accounts_ids:
                    ...
                    move_line_ids_per_partner = self.get_partners_move_lines_ids(
                        account_id, event_id, main_filter, start, stop, target_move,
            ...
            for account_id in accounts_ids:
                ...
                # We get the move line ids of the account
                move_line_ids_per_partner = self.get_partners_move_lines_ids(
                    account_id, False, main_filter, start, stop, target_move,
                    exclude_reconcile=True, partner_filter=partner_filter)
...
HeaderFooterTextWebKitParser(
    'report.account.account_report_open_invoices_without_event_webkit',
    'account.account',
    'addons/account_financial_report_webkit/report/templates/account_report_open_invoices_without_event.mako',
    parser=PartnersOpenInvoicesWebkit)
