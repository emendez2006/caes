...
class CommonPartnersReportHeaderWebkit(CommonReportHeaderWebkit):
    ...
    def get_partners_move_lines_ids(self, account_id, event_id, main_filter, start, stop,
                                    target_move,
        ...
        if filter_from:
            return self._get_partners_move_line_ids(
                filter_from, account_id, event_id, start, stop, target_move,
    ...
    def _get_partners_move_line_ids(self, filter_from, account_id, event_id, start, stop,
                                    target_move,
    ...
    def _partners_initial_balance_line_ids(self, account_ids, event_ids, start_period,
                                           partner_filter,
                                           exclude_reconcile=False,
        ...
        sql = ("SELECT ml.id, ml.account_id, ml.partner_id, ml.event_id "
        ...
        if event_ids:
            sql += "AND ml.event_id in %(event_ids)s "
            search_param.update({'event_ids': tuple(event_ids)})

        self.cursor.execute(sql, search_param)
        return self.cursor.dictfetchall()

    def _compute_partners_initial_balances(self, account_ids, event_ids, start_period,
                                           partner_filter=None,
                                           exclude_reconcile=False,
                                           force_period_ids=False):
        ...
        if event_ids and isinstance(event_ids, (int, long)):
            event_ids = [event_ids]
        else:
            event_ids = []
            
        move_line_ids = self._partners_initial_balance_line_ids(
            account_ids, event_ids, start_period, partner_filter,
            exclude_reconcile=exclude_reconcile,
            force_period_ids=force_period_ids)
        if not move_line_ids:
            move_line_ids = [{'id': -1}]
        sql = ("SELECT ml.account_id, ml.partner_id, ml.event_id,"
               ...
               "LEFT JOIN event_event ev "
               "ON ev.id = ml.event_id "
               "WHERE ml.id in %(move_line_ids)s "
               "GROUP BY ml.account_id, ml.partner_id, ml.event_id, a.currency_id, c.name")
        search_param = {
